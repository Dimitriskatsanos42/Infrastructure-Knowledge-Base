# 🐧 Linux Administration

> Σημειώσεις και labs από τη μελέτη μου στη διαχείριση Linux συστημάτων.

---

## 📋 Περιεχόμενα

| Αρχείο | Θέμα |
|--------|------|
| [`bash-scripting.md`](./bash-scripting.md) | Variables, loops, conditionals, cron jobs | 
| [`linux-advanced-administration.md`](./linux-advanced-administration.md) | systemctl, journalctl, services management |
| [`linux-basics.md`](./linux-basics.md) | Filesystem, permissions, users/groups, βασικές εντολές |
| [`systemd-services.md`](./systemd-services.md) | systemctl, journalctl, services management |
| [`systemd-services.md`](./) | systemctl, journalctl, services management |
| [`systemd-services.md`](./user-permissions-gr/md) | systemctl, journalctl, services management |


---

## 🔑 Βασικές Έννοιες

- Filesystem Hierarchy (FHS) — τι υπάρχει σε κάθε φάκελο
- Permissions (chmod/chown) — read/write/execute, octal notation
- Users & Groups — δημιουργία, διαχείριση, sudo
- systemd — πώς ξεκινούν και σταματούν services
- Bash scripting — αυτοματοποίηση επαναλαμβανόμενων εργασιών

## 🔬 Labs

- [ ] Δημιουργία users/groups και ρύθμιση permissions
- [ ] Εγκατάσταση + ρύθμιση nginx με systemctl
- [ ] Γράψιμο bash script για system health check
- [ ] Cron job για αυτόματο backup
